...
export const SigilRoot = ({sigil, onClick, note, sigilSize, scaleNotes}) => {
    ...
    var inScale = false
    if (scaleNotes.scaleNotes.includes(letter[0])) {
        inScale = true
    }
    ...
    if (isPressed){
        return(
            <div className="relative select-none" style={{ width: sigilSize, height: sigilSize }}  onClick={() => handleClick(note, inScale)} >
                ...
                <div className="absolute inset-0 flex items-center justify-center z-20">
                    ...
                    { (<span className="absolute inline-flex h-full w-full animate-ripple rounded-full 
        ...
        return(
            <div className="relative  select-none" style={{ width: sigilSize, height: sigilSize }}  onClick={() => handleClick(note, inScale)}>
                ...
                style={{ paddingBottom: '23%',  fontSize: sigilSize * 0.1 }}>{note}</span> 
                 {!inScale && (<div className="absolute rounded-full bg-black opacity-60"
                    style={{ width: '102%', height: '102%',  top: '-1%', left: '-1%' }} />)}
            </div>
        )
